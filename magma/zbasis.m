...
intrinsic OptimizedOrderBasis(KPoly::SeqEnum,ZSeq::SeqEnum[SeqEnum],a0::SeqEnum:KBestPoly:=[],KBestIsPolredabs:=false,KDisc:=0,Verbose:=false,GeneratorBoundOnly:=false) -> 
...
{ Given the coefficients of an irreducible monic polynomial f in Q[x] and a sequence of algebraic integers in the number field Q[x]/(f(x)) specified in terms of the power basis, returns:
    ...
    power_basis := Matrix([Eltseq(iota(K.1^i)) : i in [0..Degree(K)-1]]);
    ZBest := Z*power_basis;
    a0Best := Matrix(Rationals(), [a0])*power_basis;
    ...
    while cnt lt mcnt do
        ...
        assert cnt eq mcnt;
        gens := [KBest | KBest!Eltseq(ZBest[i]) : i in [1..n] | IsPrimePower(n) and not KBest!Eltseq(ZBest[n]) in Rationals()];
        ...
        OBasis := Basis(O);
        bcO := Matrix([Eltseq(KBest!b) : b in OBasis])^-1;
        ZBest_rep := ZBest*bcO;
        found := CanChangeRing(ZBest_rep, Integers());
        if found then
            ZO := ChangeRing(ZBest_rep,Integers());
            a0O := a0Best*bcO;
            break;
        else
            mcnt +:= 1;
    ...
    return Eltseq(MinimalPolynomial(KBest.1)), OBestBasis, [[r[i]:i in [1..#OBestBasis]]:r in Rows(ZOE)], Integers()!Oindex, KDisc, KBestIsPolredabs, n, [a0OE[1,i]:i in [1..#OBestBasis]]; 
end intrinsic;
