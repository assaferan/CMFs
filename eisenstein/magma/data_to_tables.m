...
// Jobs/JobId/DecomposeOnly/FormatOnly support job-splitting across many cores:
//  * worker call    (DecomposeOnly, Jobs>1, JobId in [0..Jobs-1]): runs only DecomposeSpace,
//                     which writes its slice to `infile_<JobId>`;
//  * assembly call  (FormatOnly): runs only the dimension table + Format* steps, on an `infile`
//                     that the launcher has already concatenated (and sorted) from the worker slices.
// The default (Jobs=1, neither flag) is unchanged: single-process decompose + format.
procedure DoEverythingNk2UpTo(B : folder := "../data/", do_conrey := false,
                              Jobs := 1, JobId := 0, DecomposeOnly := false, FormatOnly := false)
    ...
    // mftwists_inner and mftwists_minimal, mfsplit, mftwists_cc, were generated by Drew
    // mf_related_objects was obtained by looking up hash of objcets with the same L-function
    if not FormatOnly then
        DecomposeSpace(infile, B : Eisenstein, Jobs := Jobs, JobId := JobId);
    end if;
    if not DecomposeOnly then
        // CreateDimensionTable(infile_dim, dimfile);
        // Newer code for dimension in Drew's magma repo
